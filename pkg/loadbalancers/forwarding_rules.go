...
func (l4netlb *L4NetLB) ensureExternalForwardingRule(bsLink string) (*composite.ForwardingRule, error) {
	...
	klog.V(2).Infof("ensureExternalForwardingRule(%v): LoadBalancer IP %s", frName, ipToUse)
	...
	if !l4netlb.cloud.IsLegacyNetwork() {
		...
		noSubnetUrl := ""
		addrMgr = newAddressManager(l4netlb.cloud, nm, l4netlb.cloud.Region(), noSubnetUrl, frName, ipToUse, cloud.SchemeExternal)
		ipToUse, err = addrMgr.HoldAddress()
		if err != nil {
			return nil, err
		}
