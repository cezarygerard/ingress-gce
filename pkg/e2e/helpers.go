...
func WaitForSvcFinalilzerDeleted(s *Sandbox, svcName, finalizer string) error {
	return wait.Poll(5*time.Second, negPollTimeout, func() (bool, error) {
		svc, err := s.f.Clientset.CoreV1().Services(s.Namespace).Get(context.TODO(), svcName, metav1.GetOptions{})
		if svc == nil || err != nil {
			return true, fmt.Errorf("Failed to get service %s/%s: %v", s.Namespace, svcName, err)
		}
		if common.HasGivenFinalizer(svc.ObjectMeta, finalizer) {
			return false, nil
		}
...
func CheckServiceAttachment(sa *fuzz.ServiceAttachment, cr *sav1.ServiceAttachment) (string, error) {
	...
	if sa.GA.ConnectionPreference != cr.Spec.ConnectionPreference {
		return "", fmt.Errorf("service attachment %s connection preference does not CR %s/%s", sa.GA.ConnectionPreference, cr.Namespace, cr.Name)
	}

	var subnets []string
	for _, subnetURL := range sa.GA.NatSubnets {
		resourceID, err := cloud.ParseResourceURL(subnetURL)
		if err != nil {
			return "", fmt.Errorf("unparseable subnet url %s in gce service attachment %s", subnetURL, sa.GA.Name)
		}
		subnets = append(subnets, resourceID.Key.Name)
	}

	if !utils.EqualStringSets(subnets, cr.Spec.NATSubnets) {
		return "", fmt.Errorf("subnets in gce service attachment %s does not make CR %s/%s", sa.GA.Name, cr.Namespace, cr.Name)
	}
...
func WaitForNetLBServiceDeletion(s *Sandbox, c cloud.Cloud, svcParams *fuzz.L4LBForSvcParams) error {
	if err := WaitForSvcFinalilzerDeleted(s, svcParams.SvcName, common.NetLBFinalizerV2); err != nil {
