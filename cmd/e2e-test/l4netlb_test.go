...
func TestL4NetLbServicesNetworkTierAnnotationChanged(t *testing.T) {
	...
	Framework.RunWithSandbox("L4NetLbNetworkTier", t, func(t *testing.T, s *e2e.Sandbox) {
		...
		if _, err := e2e.EnsureL4NetLBEchoService(s, svcName, map[string]string{annotations.NetworkTierAnnotationKey: string(cloud.NetworkTierStandard)}, replicas, v1.ServiceExternalTrafficPolicyTypeCluster); err != nil {
			t.Fatalf("error ensuring echo service %s/%s: %q", s.Namespace, svcName, err)
		}
		ensureL4LbRunning(s, svcName, common.NetLBFinalizerV2, t)
		...
		if _, err := e2e.EnsureL4NetLBEchoService(s, svcName, map[string]string{annotations.NetworkTierAnnotationKey: string(cloud.NetworkTierPremium)}, replicas, v1.ServiceExternalTrafficPolicyTypeCluster); err != nil {
			t.Fatalf("error ensuring echo service %s/%s: %q", s.Namespace, svcName, err)
		}
		ensureL4LbRunning(s, svcName, common.NetLBFinalizerV2, t)
		l4netlb, err = validateNetLbService(s, svcName, t)
